...
export default function OrderCard({
  ...
  return (
    <div className="bg-white border-2 border-line rounded-chit p-5 flex flex-col gap-4 relative">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="font-mono text-sm text-muted">{order.order_code}</p>
          <p className="font-display text-3xl text-ink leading-tight mt-0.5">
        ...
        <span
          className={`shrink-0 px-3 py-1.5 rounded-full text-sm font-semibold ${STATUS_STYLES[order.status]}`}
        >
          {order.status}
        </span>
      </div>
        ...
        <div className="flex gap-3 pt-1">
          {rule.next && (
            <button
              ...
              className="flex-1 bg-ink text-paper rounded-chit py-4 text-lg font-semibold disabled:opacity-50 active:scale-[0.98] transition-transform"
            >
          ...
          {rule.canCancel && (
            <button
              ...
              className="px-5 border-2 border-accent text-accent rounded-chit py-4 text-lg font-semibold disabled:opacity-50 active:scale-[0.98] transition-transform"
            >
